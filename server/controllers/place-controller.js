...
const createPlace = async (req, res, next) => {
  ...
  try {
    ...
    const error = new HttpError("게시글 생성에 실패 하였습니다.", 500);
    return next(error);
  }
...
const deletePlace = async (req, res, next) => {
  ...
  try {
    ...
    const error = new HttpError("삭제 할 것을 찾지 못했습니다.", 500);
    return next(error);
  }
  ...
  try {
    ...
    const error = new HttpError("삭제에 실패 하였습니다.", 500);
    return next(error);
  }
...
const updatePlace = async (req, res, next) => {
  ...
  try {
    ...
    const error = new HttpError("수정 정보를 저장 하지 못했습니다.", 500);
    return next(error);
  }
  res.status(200).json({
...
const createComment = async (req, res, next) => {
  ...
  try {
    ...
    const error = new HttpError("댓글 생성에 실패 하였습니다.", 500);
    return next(error);
  }
...
const getComment = async (req, res, next) => {
  ...
  if (!comment || comment.length === 0) {
    return next(new HttpError("댓글이 없습니다.", 404));
  }
...
const deleteComment = async (req, res, next) => {
  ...
  try {
    ...
    const error = new HttpError("댓글 삭제에 실패 하였습니다.", 500);
    return next(error);
  }
